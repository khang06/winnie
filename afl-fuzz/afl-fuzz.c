/*
   ...
   Winnie forkserver by stong <stong@gatech.edu>
...
u8 use_fork = 0;                      /* Use fancy fork()                */
u8 forkserver_same_console = 0;       /* Don't create new console window */
...
static     child_pid = -1,            /* PID of the fuzzed program        */
           out_dir_fd = -1;           /* FD of the lock file              */

s32 forksrv_pid = -1;                 /* PID of the fork server           */
s32 persistent_pid = -1;
...
static u32 queued_variable,           /* Testcases with variable behavior */
...
static void bind_to_free_cpu(void) {
  ...
  if (cpu_core_count < 2) {
	WARNF("Performance is poor on single-core systems.");
	return;
  }
  ...
  // We don't set the processor affinity here; it's set on the child process by the forkserver.
  // Setting affinity for the AFL process only degrades the performance
}
...
void alloc_printf2 (char buf, char *fmt, ...) {    
    va_list va;
    va_start (va, fmt);    
    vsprintf (buf, fmt, va);    
    va_end (va); 
...
#ifdef INTELPT
  if (use_intelpt) {
	return run_target_pt(argv, timeout, init_tmout, run_dryrun);
  }
#endif
  if (use_fullspeed) {
...
static void afl_destroy_target_process() {
  if (use_fullspeed)
	destroy_target_process();
  else
	destroy_target_process_pt();
}
...
static void write_to_testcase(void* mem, u32 len) {
  ...
  if (out_file) {
    ...
    fd = open(out_file, O_WRONLY | O_BINARY | O_CREAT, 0600);

    if (fd < 0) {
      afl_destroy_target_process();
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  //printf("[*] calibrate run_target started\n");
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    fault = run_target(argv, use_tmout);     // run_target (with for calibrate-case)
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  switch (fault) {
    ...
    case FAULT_ERROR:
  		printf("WARNING: Unable to execute target application\n");
...
static double get_runnable_processes(void) {
  ...
  if (!initialized) {
	...
	if (PdhOpenQuery(NULL, NULL, &hQuery) != ERROR_SUCCESS)
	  return 0;
	if (PdhAddEnglishCounterA(hQuery, "\\System\\Processor Queue Length", NULL, &hCounter) != ERROR_SUCCESS)
	  goto fail;
  }
  ...
  for (int i = 0; i < N_SAMPLES; i++) {
	...
	PdhGetFormattedCounterValue(hCounter, PDH_FMT_LONG, NULL, &counterVal);
	max = max(counterVal.longValue, max);
  }
  return max+1;
...
static void maybe_delete_out_dir(void) {
  ...
  fn = alloc_printf("%s\\binary\\tempfile", out_dir);
  if (delete_files(fn, NULL)) goto dir_cleanup_failed;
  if (rmdir(fn) && errno != ENOENT) goto dir_cleanup_failed;

  fn = alloc_printf("%s\\binary", out_dir);
  if (delete_files(fn, NULL)) goto dir_cleanup_failed;
  ck_free(fn);
...
static void show_stats(void) {
  ...
  //printf("t_bytes: %f, t_byte_ratio: %f", t_bytes, t_byte_ratio);
  ...
  banner_len = 24 + strlen(VERSION) + strlen(WINAFL_VERSION) + strlen(WINNIE_VERSION) + strlen(use_banner);
  banner_pad = (80 - banner_len) / 2;
  memset(tmp, ' ', banner_pad);

  sprintf(tmp + banner_pad, cYEL "Winnie " cLCY WINNIE_VERSION cRST " (WinAFL " WINAFL_VERSION ", %s "
          VERSION ") " cLGN "(%s)",  crash_mode ? cPIN "PWR" : 
          "AFL", use_banner);
...
#define SP20    SP10 SP10
  ...
  if (use_fullspeed) {
	u32 visited_bbs, total_bbs;
	get_coverage_info(&visited_bbs, &total_bbs);
	sprintf(tmp, "%d/%d bbs hit", visited_bbs, total_bbs);
  } else {
	 sprintf(tmp, "%d bbs covered", t_bits);
  }
    ...
    if (skip_deterministic
        || ((!queue_cur->passed_det)
            && perf_score < (
                ...
                ? queue_cur->depth * 30
                : HAVOC_MAX_MULT * 100))
        || queue_cur->passed_det)    goto havoc_stage;
...

    {
        ...
        havoc_stage_puppet:
            ...
            for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
                ...
                if (queued_paths + unique_crashes > temp_total_found)
                {
        ...
        abandon_entry_puppet:
            ...
            if (key_puppet == 1)
            {
                if (queued_paths + unique_crashes > ((queued_paths + unique_crashes) * limit_time_bound + orig_hit_cnt_puppet))
                {
                    key_puppet = 0;
                    cur_ms_lv = get_cur_time();
                    new_hit_cnt = queued_paths + unique_crashes;
            ...
            if (tmp_pilot_time > period_pilot)
            {
...
static void handle_stop_sig(int sig) {
  SAYF("Received stop signal\n");
  if (stop_soon) { // If receive repeated Ctrl-C, REALLY exit.
	exit(1);
  }
  stop_soon = 1;
  afl_destroy_target_process();
...
static void usage(u8* argv0) {
  ...
  SAYF("\n"
	   "%s [ afl options ] -- [ instrumentation options ] -- [ forkserver options ] -- \\path\\to\\fuzzed_app [ ... ]\n\n"
...
static void setup_dirs_fds(void) {
  ...
  /* Binary and CFG information paths */

#ifdef INTELPT
  tmp = alloc_printf("%s\\binary", out_dir);
...
static void check_crash_handling(void) {
	// NOP for now
	// In the future, we could verify that minidumps are working here (TODO)
}
...
int main(int argc, char** argv) {
  ...
  SAYF("Winnie " cBRI WINNIE_VERSION cRST " -- Forkserver-based Windows fuzzer\n");
  SAYF("Based on WinAFL " cBRI WINAFL_VERSION cRST " and AFL " cBRI VERSION cRST "\n");
  SAYF("MOpt mutators from: https://github.com/puppet-meteor/MOpt-AFL \n");
  ...
  SAYF("Port of a port of modifications to a modification by Khangaroo :)\n");
  ...
  if (!in_dir || !out_dir || !timeout_given)
    usage(argv[0]);
  ...
  if (sync_id) fix_up_sync();
	...
	if (use_intelpt && use_fullspeed)
		FATAL("Intel PT and Fullspeed instrumentation are mutually exclusive");
  ...
  if (use_intelpt) {
	...
	FATAL("Unsupported instrumentation mode");
  }
  optind++;
  ...
  if (getenv("AFL_SAME_CONSOLE"))  forkserver_same_console = 1;
  ...
  binary_name = strdup(argv[optind]);
